            ...
            var deps = depsTypes.map(function (depType) {
                try {
                    ...
                    if (isInjectingUsingConstructorArguments) {
                        ...
                        throw new Error('Trying to inject ' + depTypeName + ' into ' + typeName + ' using constructor argument, but could not find binding. ' + 'Implicit binding is not possible when injecting using constructor arguments, ' + 'please att module.bind(T).to(T) for all dependencies needed by ' + typeName + '.');
                    } else {
                        throw e;
                    }
                }
